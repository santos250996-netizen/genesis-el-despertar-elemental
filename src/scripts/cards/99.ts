...
export default {
  onPlace(ctx) {
    ...
    if (col === 1 && luzAltar) {
      ctx.engine.addLog(`>> ☀ ${ctx.card.data.name} (Híbrido UMBRAL/FULGUR): enciende el Altar de la Luz.`);
    }
    if (col === 3 && sombraAltar) {
      ctx.engine.addLog(`>> 🌑 ${ctx.card.data.name} (Híbrido UMBRAL/FULGUR): enciende el Altar de la Sombra.`);
    }
  },
} as CardScript;
