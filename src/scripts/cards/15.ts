// 15 — Llamarada Voraz (FULGUR / Elemental)
...
export default {
  onPlace(ctx) {
    ...
    if (colNum === 1 && luzAltar) {
      ...
      ctx.engine.addLog(`>> ¡Efecto Celestial de Llamarada Voraz! El rival descarta 1 carta al azar.`);
      ctx.engine.compensationDraw(target);
    }
    if (colNum === 3 && sombraAltar) {
      ctx.engine.addLog(`>> ¡Efecto Umbral de Llamarada Voraz! ATK dinámico basado en diferencia de cartas en mano.`);
    }
  },
} as CardScript;
